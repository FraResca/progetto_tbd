...
describe('ProfiloUtenteAdmin', () => {
  test('renders form with empty fields when no id_paziente is provided', async () => {
    render(<ProfiloUtenteAdmin />, { wrapper: MemoryRouter })
  ...
  test('fetches and displays data when id_paziente is provided', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/paziente/1']}>
        <Routes>
          <Route path="/paziente/:id_paziente" element={<ProfiloUtenteAdmin />} />
        </Routes>
      </MemoryRouter>
    )
    ...
    await act(async () => {
      render(
        <MemoryRouter initialEntries={['/add-paziente']}>
          <Routes>
            <Route path="/add-paziente" element={<ProfiloUtenteAdmin />} />
          </Routes>
        </MemoryRouter>
      );
    });
    ...
    await act(async () => {
      render(
        <MemoryRouter initialEntries={['/edit-paziente/1']}>
          <Routes>
            <Route path="/edit-paziente/:id_paziente" element={<ProfiloUtenteAdmin />} />
          </Routes>
        </MemoryRouter>
      );
    });
