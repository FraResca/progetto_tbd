...
describe('ProfiloMedico', () => {
  test('renders form with empty fields when no id_medico is provided', async () => {
    render(<ProfiloMedicoAdmin />, { wrapper: MemoryRouter })
  ...
  test('fetches and displays data when id_medico is provided', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/medico/1']}>
        <Routes>
          <Route path="/medico/:id_medico" element={<ProfiloMedicoAdmin />} />
        </Routes>
      </MemoryRouter>
    )
    ...
    await act(async () => {
      render(
        <MemoryRouter initialEntries={['/add-medico']}>
          <Routes>
            <Route path="/add-medico" element={<ProfiloMedicoAdmin />} />
          </Routes>
        </MemoryRouter>
      );
    });
    ...
    await act(async () => {
      render(
        <MemoryRouter initialEntries={['/edit-medico/1']}>
          <Routes>
            <Route path="/edit-medico/:id_medico" element={<ProfiloMedicoAdmin />} />
          </Routes>
        </MemoryRouter>
      );
    });
